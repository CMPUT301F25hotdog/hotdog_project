 ...
 * Handles business logic for viewing entrants, running lottery draws, and sending notifications.
 *
 * <p>This controller coordinates between EventRepository and UserRepository to provide
 * complete entrant information including user details and registration dates.</p>
 *
 * <p>Controller layer in MVC pattern.</p>
 *
...
public class OrganizerEventController {
    ...
    /**
     * Loads entrants from the waiting list with their user information and join dates.
     *
     ...
     * @param listType the type of list ("waitlist", "selected", "accepted", "cancelled")
    ...
    /**
     * Helper method to get the appropriate user ID list from an event based on list type.
     *
    ...
    /**
     * Extracts the joined date for a specific event from a user's registered events.
     *
        ...
        eventRepository.getEventById(eventId, new FirestoreCallback<Event>() {
            ...
            public void onSuccess(Event event) {
                ...
                if (numberToSelect > waitlistIds.size()) {
                    callback.onError("Cannot select " + numberToSelect + " entrants. Only " + waitlistIds.size() + " in waiting list");
                    return;
                }
                ...
                eventRepository.moveEntrantsToSelected(eventId, winners, new OperationCallback() {
                    ...
                    public void onSuccess() {
                        ...
                        updateUserStatusesAndNotify(eventId, winners, losers, callback);
                    }
     ...
     */
    private void updateUserStatusesAndNotify(String eventId, List<String> winners, List<String> losers, OperationCallback callback) {
        // Send win notifications
        for (String userId : winners) {
            eventRepository.createNotification(
                    userId,
                    eventId,
                    "Congratulations! You have been selected in the lottery draw.",
        ...
        for (String userId : losers) {
            eventRepository.createNotification(
                    userId,
                    eventId,
                    "Unfortunately, you were not selected in the lottery draw. You remain on the waiting list.",
                    new OperationCallback() {
                        @Override
                        public void onSuccess() {
     ...
     * @param callback the callback to receive success or error
     */
    public void sendNotificationToEntrants(String eventId, List<String> userIds, String message, OperationCallback callback) {
        if (userIds == null || userIds.isEmpty()) {
        ...
        for (String userId : userIds) {
            eventRepository.createNotification(userId, eventId, message, new OperationCallback() {
                @Override
                public void onSuccess() {
                    successCount[0]++;
                    ...
                    if (successCount[0] + errorCount[0] == totalNotifications) {
                        if (errorCount[0] == 0) {
                            ...
                            callback.onError("Some notifications failed. " + successCount[0] + " sent, " + errorCount[0] + " failed");
                        }
                    }
                }
